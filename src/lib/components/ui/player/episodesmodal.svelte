...
<Sheet.Root {portal} bind:open={episodeListOpen}>
  <Sheet.Trigger id='episode-list-button' data-down='#player-seekbar' class='text-[rgba(217,217,217,0.6)] hover:text-neutral-500 text-sm leading-none font-light line-clamp-1 text-left hover:underline bg-transparent text-shadow-lg'>{mediaInfo.session.description}</Sheet.Trigger>
  <Sheet.Content class='w-full sm:w-[550px] p-0 max-w-full sm:max-size-full overflow-y-scroll flex flex-col !pb-0 shrink-0 gap-0 bg-black justify-between overflow-x-clip'>
